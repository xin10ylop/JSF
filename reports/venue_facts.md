...
## The contract text, and the four readings of it (2026-08-11)

The market description, verbatim (5m; 15m is identical but for the stream):

> This market will resolve to "Up" if the time-weighted average price
> (TWAP) of Bitcoin, generated by Chainlink, of the time range specified in
> the title is greater than or equal to the price at the beginning of that
> range. ... specifically the BTC/USD TWAP data stream available at
> https://data.chain.link/streams/btc-usd-twap-30s-streams. Please note
> that this market is about the price according to the TWAP Chainlink data
> stream ..., not according to any other sources or spot markets.

15m points at `btc-usd-twap-60s-streams`. That fixes w: **30s for 5m, 60s
for 15m**, which is what `MarketState.w` uses.

Both halves of the sentence are ambiguous in isolation, so all four
readings were scored against the venue's settled outcomes
(`src/verify_rule_multicoin.py`), post-change:

| reading | btc 5m | btc 15m | eth 5m | eth 15m |
|---|---|---|---|---|
| **endTwap >= kTwap** (shipped) | **0.9120** | **0.9670** | **0.9479** | **0.9721** |
| rangeTwap >= kTwap | 0.7970 | 0.8799 | 0.8169 | 0.8711 |
| endTwap >= spot0 | 0.8780 | 0.9610 | 0.9270 | 0.9617 |
| rangeTwap >= spot0 | 0.7900 | 0.8649 | 0.8065 | 0.8955 |
| spot1 >= spot0 (pre-change rule) | 0.8660 | 0.9399 | 0.8969 | 0.9477 |

The shipped reading wins every cell, and head-to-head against the
whole-range reading on markets where the two disagree it is right **87.6%,
96.8%, 90.1%, 91.4%** of the time.

So the sentence resolves as: *"the TWAP of the time range"* is the stream's
published value **at t1** — itself a w-second trailing average — and *"the
price at the beginning of that range"* is the **same stream's** value at
t0. Every quantity is a read off the TWAP stream, which is what the closing
sentence insists on; neither is raw spot. Hence

    Up  iff  mean(P over [t1-w, t1))  >=  mean(P over [t0-w, t0))

On the PRE-change rows the ordering inverts and `spot1 >= spot0` is the
best fit (0.979 / 0.990 / 0.958 / 0.990), which is the control working
backwards and further evidence the change is real.
